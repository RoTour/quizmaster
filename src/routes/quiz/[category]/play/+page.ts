...
export const load: PageLoad = async ({ params, url, fetch }) => {
  ...
  const type = (url.searchParams.get("type") ?? "RANDOM") as
    | QuizType
    | "RANDOM";
  const count = parseInt(url.searchParams.get("count") ?? "10", 10) as
    | 10
    | 35
    | 50;
  ...
  try {
    ...
    if (questions.length === 0) {
      console.error("No questions found for this configuration");
